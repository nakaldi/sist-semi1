...
public class StudentMypagePanel {
	...
	public JTextField buildingNameField;
	...
	public Panel createMyPagePanel() {
		...
		btnSaveInfoButton.addActionListener(e -> {
			...
			emailField.setEditable(false);
			emailField.setBorder(null);
			studentController.updateStudentInfo(nameField.getText(), birthDateField.getText(), phoneField.getText(),
					emailField.getText(), new String(passwordField.getPassword()));

		});

		StudentFrame.setBackgroundDisableForAllComponents(innerPanel);
		updateStudentMypage(studentController.loadStudentMypageInfo());
		return innerPanel;
	}

	public void updateStudentMypage(String[] items) {
		nameField.setText(items[0]);
		birthDateField.setText(items[1]);
		studentIdField.setText(items[2]);
		phoneField.setText(items[3]);
		emailField.setText(items[4]);
		passwordField.setText(items[5]);
		departmentNameField.setText(items[6]);
		buildingNameField.setText(items[7]);
	}
